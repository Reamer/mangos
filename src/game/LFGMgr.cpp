...
void LFGMgr::SendLFGRewards(Group* pGroup)
{
    ...
    else  if (pGroup->GetLFGGroupState()->GetType() != LFG_TYPE_RANDOM_DUNGEON)
    {
        BASIC_LOG("LFGMgr::SendLFGReward: group %u dungeon %u is not random (%u)", pGroup->GetObjectGuid().GetCounter(), pChosenDungeon->ID, pChosenDungeon->type);
        return;
    }
    ...
    for (GroupReference* itr = pGroup->GetFirstMember(); itr != NULL; itr = itr->next())
    {
        Player* pGroupMember = itr->getSource();
        if (pGroupMember && pGroupMember->IsInWorld())
            SendLFGReward(pGroupMember, pChosenDungeon);
    }
}
...
void LFGMgr::SendLFGReward(Player* pPlayer, LFGDungeonEntry const* pRandomDungeon)
{
    ...
    if (!reward)
    {
        BASIC_LOG("LFGMgr::SendLFGReward: We have no rewartd for player %u!",pPlayer->GetObjectGuid().GetCounter());
        return;
    }
    ...
    if (!qReward)
    {
        BASIC_LOG("LFGMgr::SendLFGReward quest %u is absent in DB.", reward->reward[index].questId);
        return;
    }
    ...
    // Give rewards
    BASIC_LOG("LFGMgr::SendLFGReward: %u done dungeon %u, %s previously done.", pPlayer->GetObjectGuid().GetCounter(), pRandomDungeon->ID, index > 0 ? " " : " not");
...
void LFGMgr::RemoveFomProposal(Player* pDecliner, uint32 ID)
{
    ...
    BASIC_LOG("LFGMgr::UpdateProposal: %u didn't accept. Removing from queue", pDecliner->GetObjectGuid().GetCounter());
...
void LFGMgr::OfferContinue(Group* pGroup)
{
    ...
    if (pGroup)
    {
        ...
        {
            BASIC_LOG("LFGMgr::OfferContinue: group %u not have required attributes! Status %u ", pGroup->GetObjectGuid().GetCounter(), pGroup->GetLFGGroupState()->GetStatus());
            return;
        }
        ...
        BASIC_LOG("LFGMgr::OfferContinue: no group!");
}
...
void LFGMgr::InitBoot(Player* pKicker, ObjectGuid victimGuid, std::string reason)
{
    ...
    BASIC_LOG("LFGMgr::InitBoot: group %u kicker %u victim %u reason %s", pGroup->GetObjectGuid().GetCounter(), pKicker->GetObjectGuid().GetCounter(), victimGuid.GetCounter(), reason.c_str());
...
void LFGMgr::CleanupBoots()
{
    ...
    if (!expiredBoots.empty())
    {
        ...
        {
            BASIC_LOG("LFGMgr::CleanupBoots: remove expired Boot for Group %u", (*itr).GetCounter());
...
void LFGMgr::Teleport(Group* pGroup, bool out)
{
    ...
    BASIC_LOG("LFGMgr::Teleport: %u %s dungeon!", pGroup->GetObjectGuid().GetCounter(), out ? "out" : "in");
    ...
    for (LFGQueueInfoMap::iterator infoMapitr = m_queueInfoMap.begin(); infoMapitr != m_queueInfoMap.end(); ++infoMapitr)
    {
            ...
            if (pGroup)
            {
                for (GroupReference* itr = pGroup->GetFirstMember(); itr != NULL; itr = itr->next())
                {
                    ...
                    if (pGroupMember && pGroupMember->IsInWorld())
                    {
                        ...
                        {
                            BASIC_LOG("LFGMgr::UpdateQueueStatus:Groupmember %u has no Role, this should not appear!", pGroupMember->GetObjectGuid().GetCounter());
                        }
                    }
                }
            }
        ...
        for (GuidSet::const_iterator playersitr = playerGuids.begin(); playersitr != playerGuids.end(); ++playersitr)
        {
            ...
            if (pPlayer && pPlayer->IsInWorld())
            {
                ...
                {
                    BASIC_LOG("LFGMgr::UpdateQueueStatus:Player %u has no Role, this should not appear!", pPlayer->GetObjectGuid().GetCounter());
                }
            }
        }
...
void LFGMgr::SendStatistic()
{
    for (LFGQueueInfoMap::const_iterator infoMapitr = m_queueInfoMap.begin(); infoMapitr != m_queueInfoMap.end(); ++infoMapitr)
    {
        ...
        switch (LFGType(dungeon->type))
        {
            ...
            case LFG_TYPE_NONE:
            default:
                BASIC_LOG("LFGMgr::SendStatistic: DungeonId: %u has no real type", dungeon->ID);
                return;
        }
