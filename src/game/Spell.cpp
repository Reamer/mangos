...
void Spell::SetTargetMap(SpellEffectIndex effIndex, uint32 targetMode, UnitList& targetUnitMap)
{
    ...
    switch (targetMode)
    {
        ...
        case TARGET_LEAP_FORWARD:
        {
            ...
            dest.z = std::max(loc.z, m_caster->GetTerrain()->GetWaterOrGroundLevel(dest.x, dest.y, dest.z + 8.0f));
            ...
            target->UpdateAllowedPositionZ(dest.x, dest.y, dest.z);
        ...
        case TARGET_PAA_AROUND_TARGET_OR_SELF:
        {
            ...
            m_caster->GetRandomPoint(loc.x, loc.y, loc.z, radius, destX, destY, destZ);
            loc.x = destX;
            loc.y = destY;
            loc.z = destZ;
            m_targets.setDestination(loc);
    ...
    switch (m_spellInfo->Id)
    {
        ...
        case 28374: // Decimate - Gluth encounter
        {
            ...
            if (!tempTargetUnitMap.empty())
            {
                for (UnitList::const_iterator iter = tempTargetUnitMap.begin(); iter != tempTargetUnitMap.end(); ++iter)
                {
                    if ((*iter) && ((*iter)->GetEntry() == 16360 || (*iter)->GetTypeId() == TYPEID_PLAYER))
                        targetUnitMap.push_back(*iter);
                }
            }
            break;
        ...
        case 68786:                             // Permafrost - Garfrost Encounter
        case 70336:
        {
            UnitList tmpUnitMap;
            FillAreaTargets(tmpUnitMap, radius, PUSH_DEST_CENTER, SPELL_TARGETS_AOE_DAMAGE);
            for (UnitList::const_iterator itr = tmpUnitMap.begin(); itr != tmpUnitMap.end(); ++itr)
            {
                if (*itr && m_caster->IsWithinLOSInMap(*itr))
                    targetUnitMap.push_back(*itr);
            }
            break;
        }
