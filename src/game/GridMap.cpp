...
float TerrainInfo::GetHeightTerrainInfo(float x, float y, float z, bool pUseVmaps, float maxSearchDist) const
{
...
GridMapLiquidStatus TerrainInfo::getLiquidStatus(float x, float y, float z, uint8 ReqLiquidType, GridMapLiquidData *data) const
{
    ...
    ground_level = GetHeightTerrainInfo(x,y,z,true,DEFAULT_WATER_SEARCH);
...
float TerrainInfo::GetWaterOrGroundLevel(float x, float y, float z, float* pGround /*= NULL*/, bool swim /*= false*/) const
{
    if (const_cast<TerrainInfo*>(this)->GetGrid(x, y))
    {
        ...
        float ground_z = GetHeightTerrainInfo(x, y, z, true, DEFAULT_WATER_SEARCH) + 0.05f;
        if (pGround)
            *pGround = ground_z;
...
float TerrainInfo::GetWaterLevel(float x, float y, float z, float* pGround /*= NULL*/) const
{
    ...
    {
        // we need ground level (including grid height version) for proper return water level in point
        float ground_z = GetHeightTerrainInfo(x, y, z, true, DEFAULT_WATER_SEARCH);
        if (pGround)
            *pGround = ground_z;
...
{
    return ((fabs(GetHeightTerrainInfo(x, y, oldZ, true) - oldZ) < maxDiff ) || (fabs(GetHeightTerrainInfo(x, y, oldZ, false) - oldZ) < maxDiff ));
}
