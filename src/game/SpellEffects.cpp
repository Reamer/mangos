...
void Spell::EffectInterruptCast(SpellEffectIndex eff_idx)
{
    ...
    for (uint32 i = CURRENT_FIRST_NON_MELEE_SPELL; i < CURRENT_MAX_SPELL; ++i)
    {
        if (Spell* spell = unitTarget->GetCurrentSpell(CurrentSpellTypes(i)))
        {
            ...
            // extra Spells which should be interruptible
            ...
            if (((curSpellInfo->InterruptFlags & SPELL_INTERRUPT_FLAG_INTERRUPT) && curSpellInfo->PreventionType == SPELL_PREVENTION_TYPE_SILENCE) || interruptible)
            {
            ...
            switch (m_spellInfo->SpellDifficultyId)
            {
                ...
                case 2270:                                          // Spells 73142 , 73143 , 73144 , 73145
                {
                    if (unitTarget)
                    {
                        ...
                        m_caster->CastSpell(unitTarget, m_spellInfo->CalculateSimpleValue(EFFECT_INDEX_1), true, 0, 0, m_caster->GetObjectGuid(), m_spellInfo);
                    }
                    return;
                }
